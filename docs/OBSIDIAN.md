...
narrower than its iOS one. SyncUp will surface a one-tap "Use
Obsidian/<name>" suggestion when you select the Obsidian preset on
Android.

The persistent foreground notification on Android shows a `N vaults
haven't synced recently` line whenever a vault's last sync is more than
an hour old. iOS uses local push notifications for the same signal.

If you want to script SyncUp from Tasker (start when on home WiFi, force
rescan on geofence enter, etc.), see [docs/AUTOMATION.md](AUTOMATION.md).
...
3. Pick which copy to keep, or tap **Merge…** on a `.md` conflict to open
   the markdown-aware merge view. SyncUp finds the lines that only one
   side changed and merges them automatically; lines edited by both sides
   are presented as conflict hunks where you can pick a side, keep both,
   or write a custom merge.
4. **Save merged** writes the result to the original filename and deletes
   the conflict copy.

For non-markdown conflicts the older "keep this version" path still
applies: pick a copy, the other gets deleted.
